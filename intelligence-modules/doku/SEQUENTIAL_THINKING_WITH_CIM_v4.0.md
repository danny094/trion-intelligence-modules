...
**Last Updated:** 2026-01-11 21:20 UTC
...
| **Task 3: Safety Integration** | 🔄 IN PROGRESS | 2h | 12.5% |
| **Task 4: Workflow Engine** | ⏸️ PENDING | 3h | - |
| **Task 5: Integration Tests** | ⏸️ PENDING | 2h | - |

**Phase 1 Progress:** 2.12/5 tasks complete (42.5%)  
...
#### ⏳ Step 2: Implement validate_before() (NEXT - 30 min)

**Goal:** Fully implement before-execution safety check

**What to build:**
1. **Enhanced Bias Detection**
   - Use Intelligence Loader's `check_cognitive_bias()`
   - Categorize by severity (low/medium/high/critical)
   - Return detailed issue list with corrections

2. **Cognitive Prior Checking**
   - Use Intelligence Loader's `get_relevant_priors()`
   - Apply first principles to reasoning plan
   - Flag violations of fundamental rules

3. **Smart Severity Assessment**
   - Critical biases → immediate correction needed
   - High biases → proceed with caution
   - Medium/low biases → log warning, continue

4. **Integration with Frank's Graph Selector**
   - Determine if query needs causal analysis
   - Select appropriate builder mode (auto or manual)
   - Prepare for graph construction if needed

**Code to implement:**
```python
def validate_before(self, step) -> SafetyCheck:
    # 1. Check for cognitive biases using anti-patterns
    biases = self.intelligence_loader.check_cognitive_bias(step.context)
    
    # 2. Get relevant cognitive priors
    priors = self.intelligence_loader.get_relevant_priors(step.context)
    
    # 3. Assess severity
    critical = [b for b in biases if b['severity'] == 'critical']
    high = [b for b in biases if b['severity'] == 'high']
    
    # 4. Determine action
    if critical:
        return SafetyCheck(safe=False, derailed=True, 
                          issues=critical, recommended_action="correct",
                          severity="critical")
    elif high:
        return SafetyCheck(safe=True, derailed=False,
                          issues=high, recommended_action="mitigate",
                          severity="high")
    else:
        return SafetyCheck(safe=True, derailed=False,
                          issues=[], recommended_action="proceed",
                          severity="low")
```
...
#### ⏸️ Step 4: Implement correct_course() & apply_guardrails() (PENDING - 30 min)

**Goal:** Implement correction mechanisms

**What to build:**
1. **correct_course() - Before-execution fix**
   - Use Frank's HeavyGraphBuilder for deep validation
   - Inject logic gates based on detected biases
   - Apply cognitive priors as hard constraints
   - Return corrected reasoning plan

2. **apply_guardrails() - After-execution fix**
   - Apply corrections from anti-patterns CSV
   - Inject missing validation steps
   - Add caveats/warnings to result
   - Return corrected result with annotations

**Success Criteria:**
- ✅ HeavyGraphBuilder integration working
- ✅ Logic gate injection functional
...
4. **Memory Management** (Sequential feature)
   - Cross-step state tracking
   - Variable persistence
   - Context building

5. **Budget Tracking** (Sequential feature)
   - Token counting per step
   - Time tracking per step
   - Resource limits

6. **Checkpoint System** (Sequential feature)
